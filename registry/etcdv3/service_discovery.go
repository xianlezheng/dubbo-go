...
// OnEvent
func (e *etcdV3ServiceDiscovery) OnEvent(event registry.Event) error {
	if nil != event {
...
func (e *etcdV3ServiceDiscovery) registerSreviceWatcher(serviceName string) {
	path := ROOT + constant.PATH_SEPARATOR + serviceName

	_, found := e.childListenerMap[path]

	if !found {

	}

	//etcdv3.NewEventListener(e.client).ListenServiceEvent()
}
...
func newEtcdV3ServiceDiscovery(url *common.URL) (registry.ServiceDiscovery, error) {
	...
	if err != nil {
		logger.Errorf("timeout config %v is invalid,err is %v", url.GetParam(constant.REGISTRY_TIMEOUT_KEY, constant.DEFAULT_REG_TIMEOUT), err.Error())
		return nil, perrors.WithMessagef(err, "new etcd service discovery(address:%v)", url.Location)
	}
	...
	return &etcdV3ServiceDiscovery{url, client, nil, gxset.NewSet(), make(map[string]*etcdv3.EventListener, 0)}, nil
}
